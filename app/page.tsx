...
const services = [
  ...
  { title: 'Arte digital', description: 'Dibujos, ilustraciones, iconos y portadas que cobran vida con mi estilo.', tag: 'Del boceto a la historia' },
...
const navigation = [['Portafolio', '#portfolio'], ['Servicios', '#services'], ['Sobre mí', '#about'], ['Términos', '#terms']];
  ...
  return <>
    ...
    <main id="main" className="page-shell">
      <section className="hero" aria-labelledby="hero-title"><div className="hero-copy">
        <p className="eyebrow"><span className="tiny-dot" /> ARTE DIGITAL Y CREACIONES PARA Roblox</p>
        <h1 id="hero-title">Arte a tu medida<br />para tu mundo<br className="desktop-break" /> de <span className="hero-highlight">Roblox</span><span className="lilac">.</span><Star className="title-star" /></h1>
        <p className="hero-description">Un poco de imaginación. Mucha personalidad.<br />Creo UGC, caras, dibujos y mucho más<br className="wide-only" /> para darle vida a <em>tus</em> ideas.</p>
        ...
        <p className="hero-footnote"><Heart size={12} strokeWidth={1.4} /> Un pedacito de mi mundo, hecho para el tuyo.</p>
        ...
        <div className="hero-media"><span className="media-corner top-left" aria-hidden="true" /><span className="media-corner bottom-right" aria-hidden="true" /><MediaPlaceholder label="Portada con tres personajes" /><span className="media-index" aria-hidden="true">01 / MI MUNDO CREATIVO</span></div>
      ...
      <section id="portfolio" className="portfolio-section" aria-labelledby="portfolio-title"><div className="portfolio-heading"><SectionLabel><h2 id="portfolio-title">TRABAJOS DESTACADOS</h2></SectionLabel><div className="gallery-filters" role="group" aria-label="Filtrar el portafolio">{['Todo', 'UGC', 'Caras', 'Dibujos'].map(filter => <Button key={filter} variant="ghost" className={`filter-button ${category === filter ? 'active' : ''}`} aria-pressed={category === filter} aria-controls="portfolio-gallery" onClick={() => setCategory(filter)}>{filter}</Button>)}</div><span className="gallery-aside">un vistazo a mi mundo <Star /></span></div>
        ...
        <p className="gallery-note">Algo bonito está tomando forma. Pronto verás mis creaciones aquí.</p>
      </section>

      <div className="about-terms"><section id="about" className="about-section" aria-labelledby="about-title"><SectionLabel><h2 id="about-title">SOBRE MÍ</h2></SectionLabel><div className="about-content"><div className="about-art"><div className="about-outline" aria-hidden="true" /><div className="about-media"><MediaPlaceholder label="Artista / avatar" small /></div><Star className="about-star" /><span className="about-sticker" aria-hidden="true">un poquito de mí ♡</span></div><div className="about-copy"><h3>Hola, soy Jolbzie<span className="lilac">.</span> <span className="greeting-star" aria-hidden="true">✳</span></h3><p>Soy artista digital y creo para Roblox. Me encanta convertir pequeñas ideas en creaciones llenas de personalidad.</p><p>Ya sea UGC, caras o ilustraciones, cuido los detalles para crear algo original que se sienta muy <em>tuyo</em>.</p><p className="about-thanks">Gracias por estar aquí. Creemos algo que te encante.</p><span className="signature handwritten">— Jolbzie ♡</span></div></div></section>
      ...
      <section id="commission" className="commission-section" aria-labelledby="commission-title"><div className="cta-orbit orbit-one" aria-hidden="true" /><div className="cta-orbit orbit-two" aria-hidden="true" /><Star className="cta-star-one" /><Star className="cta-star-two" /><span className="cta-small-star" aria-hidden="true">✧</span><p className="eyebrow">TU IDEA. MI CREATIVIDAD. UN TOQUE DE MAGIA.</p><h2 id="commission-title">Creemos algo <span className="handwritten">muy tuyo.</span></h2><p>¿Tienes un personaje, un concepto o una idea en mente?</p><div className="cta-buttons"><Button className="button button-primary" onClick={() => setDialog('commission')}>Pedir una comisión <ArrowUpRight size={16} /></Button><Button className="button button-outline" variant="outline" onClick={() => setDialog('pricing')}>Ver precios y condiciones</Button></div><span className="cta-love" aria-hidden="true">♡</span></section>
    </main>

    <footer className="site-footer"><div className="footer-inner"><div><a className="wordmark" href="#">DB_JOLBZIE<span>✦</span></a><p>Pequeñas ideas. Un mundo de posibilidades.</p></div><nav aria-label="Navegación del pie de página">{navigation.map(([label, href]) => <a href={href} key={href}>{label}</a>)}</nav><FooterSocialLinks onSelect={() => setDialog('social')} /></div><div className="footer-bottom"><span>© {new Date().getFullYear()} DB_JOLBZIE</span><span>Hecho con cariño y un poco de magia <span className="lilac">✦</span></span></div></footer>

    <Dialog open={dialog !== null} onOpenChange={open => { if (!open) setDialog(null); }}><DialogContent className="artist-dialog"><span className="dialog-sparkle" aria-hidden="true">✦</span><p className="eyebrow">ESTUDIO DB_JOLBZIE</p><DialogTitle className="dialog-title">{dialog === 'commission' ? 'Tu idea empieza aquí.' : dialog === 'pricing' ? 'Todo claro desde el inicio.' : 'Sigamos en contacto.'}</DialogTitle><DialogDescription className="dialog-description">{dialog === 'commission' ? 'Pronto podrás pedir tu comisión. Aquí encontrarás los datos de contacto y la disponibilidad cuando todo esté listo.' : dialog === 'pricing' ? 'Estoy preparando los precios y las condiciones de las comisiones. Podrás consultarlos aquí antes de que se abran los pedidos.' : 'Pronto encontrarás aquí los enlaces a mis redes oficiales. Vuelve para descubrir un poco más de mi mundo creativo.'}</DialogDescription><div className="dialog-note"><Heart size={16} />{dialog === 'commission' ? 'Mientras tanto, imagina tu personaje, su estilo y esos detalles que lo hacen único.' : 'Gracias por ser parte de este rincón creativo.'}</div><Button className="button button-primary" onClick={() => { setDialog(null); document.getElementById(dialog === 'pricing' ? 'terms' : 'services')?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' }); }}>{dialog === 'pricing' ? 'Ver condiciones' : 'Ver lo que ofrezco'}<ArrowDown size={15} /></Button></DialogContent></Dialog>
  </>;
}
